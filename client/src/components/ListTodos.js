...
const ListTodos = () => {
    ...
    return (
        <div>
            {isOpen && <EditTodo setIsOpen={setIsOpen} currentTodo={currentTodo}/>}
                    ...
                    <tbody className="tbl__body" >
                        {todos && todos.map((todo, idx) => (
                            <tr key={todo.todo_id} className="d-flex">
                                ...
                                <td className="col-2 text-left"><button className="btn btn-success px-4" onClick={() => handleEdit(todo)}>Edit</button></td>
